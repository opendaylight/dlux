...
function run_pep8 {
  echo "Running flake8 ..."
  DJANGO_SETTINGS_MODULE=dlux.test.settings ${command_wrapper} flake8 $included_dirs
}
...
function run_tests_all {
  ...
  export NOSE_XUNIT_FILE=dlux/nosetests.xml
  if [ "$NOSE_WITH_HTML_OUTPUT" = '1' ]; then
    export NOSE_HTML_OUT_FILE='dlux_nose_results.html'
  fi
  ${command_wrapper} coverage erase
  ${command_wrapper} coverage run -p $root/manage.py test dlux --settings=dlux.test.settings $testopts
