...
// Will use logger after implementing and testing
import org.opendaylight.dlux.bootstrapper.api.IDluxBootstrapperRegistration;

public class DluxCore implements BundleActivator {
    ...
    public void registerCore() throws NamespaceException{
        // MOVE to a class?
        if(httpService != null) {
            httpService.registerResources("/core", "/core", null);
        } else {
            System.out.println("httpService is null. Cannot register resources");
        }
        if (httpService != null && bootstrapper != null) {
            bootstrapper.addModule("core", "/core", "core.module", " app.module");
        } else {
            System.out.println("bootstrapper is null. Cannot add core module to bootstrapper");
        }
    }
}
