angular.module('common.sfc.api', [])
  ...
  // ******* SfcRestBaseSvc *********
  .factory('SfcRestBaseSvc', function (Restangular, RpcError) {
    ...
    SfcRestBaseSvc.prototype.baseRpcRest = function () {
      return Restangular.one('operations');
    };
    ...
    SfcRestBaseSvc.prototype.executeOperation = function (input, operationName, params, callback) {
      var instance = this; // save 'this' to closure

      this.postRpc(input, operationName, params).then(function (result) {
        callback(result); // return response
      }, /* on error*/ function (response) {
        callback(new RpcError(response)); // RpcError
      });
    };
  ...
  .factory('ServiceChainSvc', function (SfcRestBaseSvc) {
    ...
    ServiceChainSvc.prototype.deployChain = function (sfcName, callback) {
      ...
      this.executeOperation(input, "instantiate-service-function-chain", undefined, callback);
    };
