package org.opendaylight.dlux.network;
/**
...
public class DluxNetwork {
    ...
    public void registerCore() throws NamespaceException{
        if(httpService != null) {
            ...
            httpService.registerResources("/network", "/network", null);
        } else {
            //logger.error("httpService is null. Cannot register resources");
        }
        if (httpService != null && bootstrapper != null) {
            bootstrapper.addModule("network", "/network", "network", "console.networking");
        } else {
