define(['app/topology/topology.module'], function(topology) {
  ...
  topology.register.factory('NetworkTopologySvc', function(TopologyRestangular) {
    ...
    topology.register.factory('MDSalRestangular', function(Restangular, ENV) {
        return Restangular.withConfig(function(RestangularConfig) {
            RestangularConfig.setBaseUrl(ENV.getBaseURL("MD_SAL"));
        });
    });

    topology.register.factory('ADSalRestangular', function(Restangular, ENV) {
        return Restangular.withConfig(function(RestangularConfig) {
            RestangularConfig.setBaseUrl(ENV.getBaseURL("AD_SAL"));
        });
    });

    topology.register.factory('pollSvc', function(MDSalRestangular,ADSalRestangular) {
        var svc = {
            mdsalbase: function() {
        ...
        svc.startPoll = function(url,typename,timeout){
            setTimeout(function() {
                ...
                else if(["yangui", "yangvisualizer"].indexOf(typename) > -1){
