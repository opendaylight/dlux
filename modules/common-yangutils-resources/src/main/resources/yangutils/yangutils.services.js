define(['common/yangutils/yangutils.module'], function (yangUtils) {
    ...
    yangUtils.factory('mountPointsConnector', function (YangUIApis, nodeWrapper, yangUtils, constants, eventDispatcher, apiBuilder, yinParser, pathUtils) {
        ...
        mp.cutMountPointPathFromPathString = function(path) {
            var configStr = 'restconf/config',
                operStr = 'restconf/operational',
                cutEnd = path.indexOf(mountPrefix) + mountPrefix.length,
                pathToMount = path.slice(0, cutEnd),
                cutStart = 0;

            if(path.indexOf(configStr) !== -1) {
                cutStart = path.indexOf(configStr) + configStr.length;
            } else if(path.indexOf(operStr) !== -1) {
                cutStart = path.indexOf(operStr) + operStr.length;
            }

            var processedPath = path.slice(0, cutStart) + path.slice(cutEnd);
