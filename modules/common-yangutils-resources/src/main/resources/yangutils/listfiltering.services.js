define(['common/yangutils/listfiltering.module'], function (listFiltering) {
    ...
    listFiltering.factory('nodeWrapperForFilter', function(constants) {
        ...
        nodeWrapperForFilter.init = function(node){
            ...
            node.deepCopyForFilter = function (additionalProperties) {
                ...
                var copy = node.getCleanCopy(),
                    self = this,
                    allowedTypesForFilterType = ['string','uint32','uint8','decimal64','int16','int32','int64','int8','uint16','uint64','union','bits'],
                    ...
                    addFilterProps = function(childrenArray){
                        if(childrenArray.type == 'leaf' && childrenArray.children && childrenArray.children.length){
                            if(childrenArray.children.some(function(item){
                                return item.type == 'type' && allowedTypesForFilterType.indexOf(item.label) != -1;
                            })){
    ...
    listFiltering.factory('listFiltering', function (nodeWrapperForFilter, reqBuilder) {
        ...
        var getNodePathInStructure = function (selSubApi,node) {
                var iterator = -1,
            ...
            if(selSubApi.node != node){
                getSearchedPath(selSubApi.node);
            }
        },
        ...
        listFiltering.showListFilterWin = function (selSubApi,node) {
            if(!node.searchedPath.length){
                getNodePathInStructure(selSubApi,node);
            }

            if(!node.referenceNode){
                node.referenceNode = selSubApi.node;
        ...
        listFiltering.getFilterTypeArray = function (type) {
            if(type == 'number'){
                return ['=','>','>=','<','<=','range'];
            }else{
