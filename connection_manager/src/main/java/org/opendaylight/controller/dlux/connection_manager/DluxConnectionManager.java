...
public class DluxConnectionManager {
    HttpService httpService;
    IDluxBootstrapperRegistration bootstrapper;
    final static Logger logger = LoggerFactory.getLogger(DluxConnectionManager.class);
    ...
    public void onBindHttpService(final HttpService srv) throws NamespaceException {
        if (srv == null) {
            ...
            logger.debug("HttpService injected");
    ...
    public void onBindBootService(final IDluxBootstrapperRegistration bootstrapper) throws NamespaceException {
        if (bootstrapper == null) {
            ...
            logger.debug("bootstrapperService is not null");
    ...
    public void registerConnectionManager() throws NamespaceException{
        if(httpService != null) {
            logger.info("Registering resources for connection_manager");
            httpService.registerResources("/connection_manager", "/connection_manager", null);
        } else {
            logger.error("httpService is null. Cannot register resources for connection_manager");
        }
        if (httpService != null && bootstrapper != null) {
            bootstrapper.addModule("connection_manager", "/connection_manager", "connection_manager.module", "app.connection_manager");
        } else {
            logger.error("bootstrapper is null. Cannot add module to bootstrapper");
        }
    }
}
